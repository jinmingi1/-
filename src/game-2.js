...
import { Wiper } from "./wiper-2.js";
import { Vector2 } from "./vector2-2.js";
import { Ball } from "./ball-2.js";
...
export class Game {
  ...
  update(delta) {
    ...
    for (let i = 0; i < this.stepDivision; i++) {
      ...
      this.balls.forEach(b => {
        ...
        const baseToBall = b.pos.subV(mid);
        const bound = this.wiper.dir.dot(baseToBall);

        if (bound < 0) {
          let dir = b.pos.subV(mid);
          const dist = dir.len();
  ...
  handleImageClick(imageUrl) {
    // 여기서 이미지 클릭 처리를 하세요.
    // 클릭된 이미지에 해당하는 작업을 수행하세요.
    // imageUrl을 활용하여 필요한 동작을 추가하세요.
